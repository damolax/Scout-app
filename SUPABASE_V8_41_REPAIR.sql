-- Scout v8.41 repair: schedules, follow-ups, signatures, and simple audience categories.
-- Run once in Supabase SQL Editor, then wait ~30 seconds and refresh the app.
...
-- Workspace/admin setup columns used by the simplified Settings page and extension.
...
-- Audience categories stay intentionally simple.
...
select w.id, 'Airtable', 'Audience/template category for Airtable-related outreach.', true
from public.workspaces w
on conflict (workspace_id, name) do nothing;

insert into public.message_categories (workspace_id, name, description, active)
select w.id, 'Shopify', 'Audience/template category for Shopify-related outreach.', true
from public.workspaces w
on conflict (workspace_id, name) do nothing;
...
create index if not exists message_schedules_workspace_stop_idx
on public.message_schedules(workspace_id, stop_requested, status);
...
create index if not exists reply_history_workspace_business_received_idx
on public.reply_history(workspace_id, business_id, received_at desc);

create index if not exists businesses_workspace_category_status_idx
on public.businesses(workspace_id, category_id, status, updated_at desc);

-- Follow-up RPC used by Message, Dashboard, Operations, and worker.
