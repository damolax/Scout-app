...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: inserted, error } = await admin.from('businesses').upsert(payload, {
    ...
    await admin.from('activity_logs').insert({
      ...
      message: `Extension imported ${insertedRows.length} lead(s)${defaultCategoryName ? ` for ${defaultCategoryName}` : ''}, direct emails ${directEmailRows.length}, queued ${queuedAutoScout} website(s) for Auto Scout.`,
      raw: { received: rows.length, inserted: insertedRows.length, directEmails: directEmailRows.length, queuedAutoScout, source: body.source || 'extension', audienceCategoryId: defaultCategoryId, audienceCategoryName: defaultCategoryName }
    });
    ...
    return NextResponse.json({
      ...
      skippedOrDuplicate: Math.max(0, payload.length - insertedRows.length),
      duplicates: Math.max(0, payload.length - insertedRows.length),
