...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const payload = {
      ...
      health_score: Number(existing?.health_score ?? 100),
      health_reliability: existing?.health_reliability || 'insufficient_evidence',
      owner_override_limit: existing?.owner_override_limit || null,
      owner_override_active: Boolean(existing?.owner_override_active),
      owner_override_until: existing?.owner_override_until || null,
      owner_override_locked: Boolean(existing?.owner_override_locked),
      harmful_override_streak: Number(existing?.harmful_override_streak || 0),
      recovery_step: Number(existing?.recovery_step || 0),
      health_reason: existing?.health_reason || 'New sender is assessed at a 250/day system ceiling. The owner may select any lower daily limit.',
