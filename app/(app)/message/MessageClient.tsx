...
export default function MessageClient({ workspace }: { workspace: Workspace }) {
  ...
  const [summary, setSummary] = useState<Summary>({ requested: 0, attempted: 0, sent: 0, failed: 0, skipped: 0, stopped: false });
  ...
  async function loadAccounts() {
    const { data, error: loadError } = await supabase.from('gmail_accounts').select('*').eq('workspace_id', workspace.id).order('created_at', { ascending: false });
    if (loadError) throw loadError;
  ...
  async function loadReadyContacts() {
    ...
    if (targetBusinessId) {
      const { data: target, error: targetError } = await supabase.from('businesses').select('*').eq('workspace_id', workspace.id).eq('id', targetBusinessId).maybeSingle();
  ...
  async function loadRecentSent() {
    const { data } = await supabase.from('sent_messages').select('id,status,to_email,from_email,subject,sent_at').eq('workspace_id', workspace.id).order('sent_at', { ascending: false }).limit(80);
  ...
  async function loadSchedules() {
    const { data, error: scheduleError } = await supabase.from('message_schedules').select('*').eq('workspace_id', workspace.id).in('status', ['scheduled', 'due', 'running']).order('scheduled_for', { ascending: true }).limit(50);
  ...
  async function refreshAll() {
    ...
    setError('');
    try {
      await Promise.all([loadCategories(), loadTemplates(), loadAccounts(), loadReadyContacts(), loadRecentSent(), loadDueFollowUps(), loadSchedules()]);
  ...
  useEffect(() => {
    const first = templates.find((t) => t.category_id === categoryId) || templates[0];
    if (first && !templates.some((t) => t.id === templateId && (!categoryId || t.category_id === categoryId))) setTemplateId(first.id);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  function accountsForSend() {
    if (senderMode === 'specific') return connectedAccounts.filter((a) => a.id === specificSenderId);
  ...
  function buildContactPayload(business: Business, template: MessageTemplate, index: number) {
  ...
  async function persistSendOutcome(params: { business: Business; template: MessageTemplate; account: GmailAccount; result: SendResult; batchId: string; subject: string; body: string; dryRun: boolean; isFollowUp?: boolean }) {
    ...
    if (isSent && !isDryRun) {
      await supabase.from('businesses').update({ status: 'contacted', updated_at: sentAt }).eq('workspace_id', workspace.id).eq('id', business.id);
  ...
  async function sendBatch(contactsOverride?: Business[], options?: { isFollowUp?: boolean; limit?: number }) {
    ...
    try {
      ...
      await Promise.all([loadReadyContacts(), loadAccounts(), loadRecentSent(), loadDueFollowUps()]);
    } catch (err) {
      setError(formatError(err));
    } finally {
      setBusy(false);
  ...
  return (
    <div className="stack">
      ...
      <div className="grid grid-4">
        <div className="card kpi"><div className="title">Ready</div><div className="num">{readyTotal.toLocaleString()}</div></div>
        <div className="card kpi"><div className="title">Connected Senders</div><div className="num">{connectedAccounts.length}</div></div>
      ...
      <div className="card" style={{ padding: 18 }}>
        ...
        <div className="grid grid-2" style={{ marginTop: 14 }}>
          <div className="card" style={{ padding: 14 }}>
            ...
            <label className="checkbox-row"><input type="radio" checked={templateMode === 'specific'} onChange={() => setTemplateMode('specific')} /> Use selected template</label>
            <label className="checkbox-row"><input type="radio" checked={templateMode === 'rotate'} onChange={() => setTemplateMode('rotate')} /> Rotate all templates in selected category</label>
            <select className="select" value={templateId} onChange={(e) => setTemplateId(e.target.value)} disabled={templateMode === 'rotate'}>
              <option value="">Select template</option>
              {(categoryId ? categoryTemplates : templates).map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}
            </select>
            <div className="actions" style={{ marginTop: 10 }}><Link className="btn secondary" href="/templates">Manage Templates</Link></div>
          </div>
