      ...
      <div className="card" style={{ padding: 18 }}>
        <div className="actions" style={{ justifyContent: 'space-between' }}>
          <div>
            ...
            <p className="muted" style={{ marginBottom: 0 }}>Automatic Gmail inbox and reply reading is disabled in this send-only Google-verification build. Scout does not scan unrelated inbox messages.</p>
          </div>
          <button className="btn secondary" onClick={() => loadAll().catch((err) => setError(formatError(err)))} disabled={busy}>Refresh stored records</button>
        </div>
        <div className="notice" style={{ marginTop: 12 }}>After advanced Google authorization is approved, the app-scoped sync can be enabled. That sync checks only Gmail thread IDs created by Scout-sent messages and recipient-specific delivery notices.</div>
        <div className="actions" style={{ marginTop: 12 }}>
          {replyRows.length ? <button className="btn secondary" type="button" onClick={() => downloadCsv('scout-replies.csv', realReplies as unknown as Array<Record<string, unknown>>)}>Export stored replies</button> : null}
          {noInboxRows.length ? <button className="btn secondary" type="button" onClick={() => downloadCsv('scout-no-inbox.csv', noInboxRows as unknown as Array<Record<string, unknown>>)}>Export stored no-inbox records</button> : null}
        </div>
      </div>
