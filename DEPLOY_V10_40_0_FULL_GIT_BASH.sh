...
BRANCH="main"
WORK_DIR="${HOME}/scout-v10-40-deploy"
EXPECTED_VERSION="10.40.0"
...
grep -Fq "$REPO_URL" "$SOURCE_DIR/DEPLOY_V10_40_0_FULL_GIT_BASH.sh" || fail "Deployment target verification failed."
...
BACKUP_BRANCH="backup-before-v10-40-$(date +%Y%m%d-%H%M%S)"
...
if git diff --cached --quiet; then
  ...
  git commit -m "Deploy Scout v10.40.0 full replies and signature sync"
...
if command -v curl >/dev/null && [ -n "$LIVE_URL" ]; then
  ...
  for _ in $(seq 1 36); do
    ...
    if printf '%s' "$BODY" | node -e '
      let s=""; process.stdin.on("data",d=>s+=d); process.stdin.on("end",()=>{try{const j=JSON.parse(s); process.exit(j?.version==="10.40.0" && j?.schema?.contractVersion==="10.40.0" && j?.ready===true ? 0 : 1)}catch{process.exit(1)}});'; then
      VERIFIED="true"
      break
    fi
    sleep 10
...
echo "Replies and signature sync are ready for controlled live acceptance tests."
