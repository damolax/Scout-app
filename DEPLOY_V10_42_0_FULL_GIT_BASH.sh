...
EXPECTED_VERSION="10.42.0"
EXPECTED_BUILD="background-import-adaptive-sender-health-permanent-xp"
...
[ -f "$SOURCE_DIR/RUN_THIS_V10_42_UPGRADE_IN_CURRENT_SUPABASE.sql" ] || fail "v10.42 current-project SQL is missing."
...
grep -Fq "$REPO_URL" "$SOURCE_DIR/DEPLOY_V10_42_0_FULL_GIT_BASH.sh" || fail "Deployment target verification failed."
...
if git diff --cached --quiet; then
  ...
  git commit -m "Add background imports, adaptive sender health, and permanent Scout XP"
...
if command -v curl >/dev/null && [ -n "$LIVE_URL" ]; then
  ...
  for _ in $(seq 1 42); do
    ...
    if printf '%s' "$BODY" | node -e '
      let s=""; process.stdin.on("data",d=>s+=d); process.stdin.on("end",()=>{try{const j=JSON.parse(s);process.exit(j?.version==="10.42.0"&&j?.build==="background-import-adaptive-sender-health-permanent-xp"&&j?.bulkImportReady===true&&j?.schema?.contractVersion==="10.42.0"?0:1)}catch{process.exit(1)}});'; then
      VERIFIED="true"
      break
    fi
    sleep 10
  ...
  if [ "$VERIFIED" != "true" ]; then
    echo
    echo "GitHub is updated, but live v10.42 verification is not ready."
    echo "Check Vercel Deployments, then verify the v10.42 SQL and import-worker cron using the HTML wizard."
    exit 2
  fi
fi
...
echo "Schema contract:          10.42.0"
echo "Background import worker: included"
