...
EXPECTED_VERSION="10.42.1"
EXPECTED_BUILD="fast-direct-import-run-100-pacing-hotfix"
...
[ -f "$SOURCE_DIR/RUN_THIS_V10_42_1_HOTFIX_IN_CURRENT_SUPABASE.sql" ] || fail "v10.42.1 hotfix SQL is missing."
...
grep -Fq "$REPO_URL" "$SOURCE_DIR/DEPLOY_V10_42_1_FULL_GIT_BASH.sh" || fail "Deployment target verification failed."
...
if git diff --cached --quiet; then
  ...
  git commit -m "Restore fast imports and set sender run default to 100"
...
if command -v curl >/dev/null && [ -n "$LIVE_URL" ]; then
  ...
  for _ in $(seq 1 42); do
    ...
    if printf '%s' "$BODY" | node -e '
      let s=""; process.stdin.on("data",d=>s+=d); process.stdin.on("end",()=>{try{const j=JSON.parse(s);process.exit(j?.version==="10.42.1"&&j?.build==="fast-direct-import-run-100-pacing-hotfix"&&j?.bulkImportReady===true&&j?.schema?.contractVersion==="10.42.1"?0:1)}catch{process.exit(1)}});'; then
      VERIFIED="true"
      break
    fi
    sleep 10
  ...
  if [ "$VERIFIED" != "true" ]; then
    echo
    echo "GitHub is updated, but live v10.42.1 verification is not ready."
    echo "Check Vercel Deployments, then verify the v10.42.1 hotfix SQL and existing worker configuration using the HTML wizard."
    exit 2
  fi
fi
