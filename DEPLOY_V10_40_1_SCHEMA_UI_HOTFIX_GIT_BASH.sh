...
BRANCH="main"
WORK_DIR="${HOME}/scout-v10-40-1-deploy"
EXPECTED_VERSION="10.40.1"
...
grep -Fq "$REPO_URL" "$SOURCE_DIR/DEPLOY_V10_40_1_SCHEMA_UI_HOTFIX_GIT_BASH.sh" || fail "Deployment target verification failed."
...
BACKUP_BRANCH="backup-before-v10-40-1-schema-ui-hotfix-$(date +%Y%m%d-%H%M%S)"
...
if git diff --cached --quiet; then
  ...
  git commit -m "Fix Scout template schema check and full Gmail wording"
...
if command -v curl >/dev/null && [ -n "$LIVE_URL" ]; then
  ...
  for _ in $(seq 1 36); do
    ...
    if printf '%s' "$BODY" | node -e '
      let s=""; process.stdin.on("data",d=>s+=d); process.stdin.on("end",()=>{try{const j=JSON.parse(s); process.exit(j?.version==="10.40.1" && j?.build==="full-replies-signature-schema-ui-hotfix" && j?.schema?.contractVersion==="10.40.0" && j?.ready===true ? 0 : 1)}catch{process.exit(1)}});'; then
      VERIFIED="true"
      break
    fi
    sleep 10
  ...
  if [ "$VERIFIED" != "true" ]; then
    ...
    echo "The usual causes are: Vercel is still building, the v10.40 SQL was not run, or environment variables are incomplete."
    exit 2
  fi
fi
...
echo "Template schema readiness, full Gmail reply collection, and signature synchronization are ready for controlled live acceptance tests."
