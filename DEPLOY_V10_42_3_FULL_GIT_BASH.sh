...
BRANCH="main"
WORK_DIR="${HOME}/scout-v10-42-3-deploy"
EXPECTED_VERSION="10.42.3"
EXPECTED_BUILD="auto-scout-contact-page-trust-fix"
...
grep -Fq "$REPO_URL" "$SOURCE_DIR/DEPLOY_V10_42_3_FULL_GIT_BASH.sh" || fail "Deployment target verification failed."
...
BACKUP_BRANCH="backup-before-v10-42-3-$(date +%Y%m%d-%H%M%S)"
...
if git diff --cached --quiet; then
  ...
  git commit -m "Fix Auto Scout contact discovery and trust filtering"
...
if command -v curl >/dev/null && [ -n "$LIVE_URL" ]; then
  ...
  for _ in $(seq 1 42); do
    ...
    if printf '%s' "$BODY" | node -e '
      let s=""; process.stdin.on("data",d=>s+=d); process.stdin.on("end",()=>{try{const j=JSON.parse(s);process.exit(j?.version==="10.42.3"&&j?.build==="auto-scout-contact-page-trust-fix"&&j?.bulkImportReady===true&&j?.schema?.contractVersion==="10.42.2"?0:1)}catch{process.exit(1)}});'; then
      VERIFIED="true"
      break
    fi
    sleep 10
  ...
  if [ "$VERIFIED" != "true" ]; then
    echo
    echo "GitHub is updated, but live v10.42.3 verification is not ready."
    echo "Check Vercel Deployments, then verify the existing v10.42.2 schema and worker configuration using the HTML wizard."
    exit 2
  fi
fi
...
echo "Auto Scout trust fix:     included"
