...
-- Secure, staged follow-up queue.
-- A business can receive Follow-up 1 once and Follow-up 2 once. After that the
-- sequence is complete. Failed sends do not advance the stage and remain retryable.
...
drop function if exists public.count_due_followups(uuid, text, integer);
drop function if exists public.count_due_followups(uuid, text, integer, integer);
drop function if exists public.count_due_followups(uuid, text, integer, integer, uuid);
...
drop function if exists public.get_due_followups(uuid, integer, text, integer);
drop function if exists public.get_due_followups(uuid, integer, text, integer, integer);
drop function if exists public.get_due_followups(uuid, integer, text, integer, integer, uuid);
...
as $$
  ...
  select
    ...
    ls.template_id,
    ls.gmail_account_id,
    (mr.followup_count + 1)::integer as followup_stage,
    mr.followup_count::integer as previous_followups,
    mr.last_sent_at + make_interval(hours => greatest(1, least(720, $5))) as next_eligible_at,
  ...
  where b.workspace_id = target_workspace
    ...
    and mr.followup_count = greatest(1, least(2, $4)) - 1
    and mr.last_sent_at <= now() - make_interval(hours => greatest(1, least(720, $5)))
    ...
    and (
      coalesce(trim($7), '') = ''
      or lower(coalesce(b.location, '')) like '%' || lower(trim($7)) || '%'
      or lower(coalesce(b.raw::text, '')) like '%' || lower(trim($7)) || '%'
    )
...
select
  ...
  '90-210 seconds'::text as same_gmail_delay,
  '3-6 seconds'::text as different_gmail_delay,
  to_regprocedure('public.count_due_followups(uuid,text,integer,integer,uuid,text)') is not null as all_followups_ready;
...
-- ============================================================================
-- SCOUT v10.39.0 SETTINGS + DEPLOYED-SCHEMA CONTRACT
-- Idempotent: safe to run again after future code deployments.
-- ============================================================================

alter table if exists public.gmail_accounts add column if not exists access_token text;
alter table if exists public.gmail_accounts add column if not exists refresh_token text;
...
alter table if exists public.sent_messages add column if not exists from_email text;
...
alter table if exists public.sent_messages add column if not exists follow_up_stage integer not null default 0;

alter table if exists public.reply_history add column if not exists sent_message_id uuid;
alter table if exists public.reply_history add column if not exists from_email text;
alter table if exists public.reply_history add column if not exists snippet text;
...
alter table if exists public.message_schedules add column if not exists scheduled_for timestamptz;
alter table if exists public.message_schedules add column if not exists target_count integer not null default 0;
alter table if exists public.message_schedules add column if not exists processed_count integer not null default 0;
alter table if exists public.message_schedules add column if not exists sent_count integer not null default 0;
alter table if exists public.message_schedules add column if not exists failed_count integer not null default 0;
alter table if exists public.message_schedules add column if not exists stop_requested boolean not null default false;

alter table if exists public.activity_logs add column if not exists created_by uuid;
...
alter table public.scout_schema_versions enable row level security;

insert into public.scout_schema_versions (version, applied_at, notes)
values (
  '10.39.0',
  now(),
  'Settings simplification, live schema guard, staged follow-up contract, and simplified replies.'
)
...
select
  'READY'::text as scout_database_status,
  '10.39.0'::text as deployed_schema_contract,
  exists (
    select 1 from public.scout_schema_versions where version = '10.39.0'
  ) as schema_version_recorded,
  ...
  to_regprocedure('public.scout_message_worker_status()') is not null as worker_status_ready;



-- =============================================================================
-- SCOUT v10.40.0 FULL REPLY + SIGNATURE CONTRACT
-- Idempotent: safe after the current Scout upgrade SQL.
-- =============================================================================
create extension if not exists pgcrypto;
...
alter table if exists public.gmail_accounts add column if not exists granted_scopes text[] not null default '{}'::text[];
...
-- Remove historical duplicate Gmail message rows before enforcing uniqueness.
with ranked as (
  select id, row_number() over (partition by workspace_id, gmail_message_id order by received_at desc nulls last, id) as rn
  from public.reply_history
  where gmail_message_id is not null and btrim(gmail_message_id) <> ''
)
delete from public.reply_history where id in (select id from ranked where rn > 1);

with ranked as (
  select id, row_number() over (partition by workspace_id, gmail_message_id order by created_at desc nulls last, id) as rn
  from public.no_inbox_records
  where gmail_message_id is not null and btrim(gmail_message_id) <> ''
)
delete from public.no_inbox_records where id in (select id from ranked where rn > 1);

create unique index if not exists reply_history_workspace_gmail_message_uidx
  on public.reply_history(workspace_id, gmail_message_id)
  where gmail_message_id is not null and btrim(gmail_message_id) <> '';
create unique index if not exists no_inbox_workspace_gmail_message_uidx
  on public.no_inbox_records(workspace_id, gmail_message_id)
  where gmail_message_id is not null and btrim(gmail_message_id) <> '';
create index if not exists gmail_accounts_reply_sync_order_idx
  on public.gmail_accounts(last_reply_sync_at nulls first, updated_at);
create index if not exists sent_messages_workspace_thread_idx
  on public.sent_messages(workspace_id, gmail_account_id, gmail_thread_id)
  where gmail_thread_id is not null;

update public.gmail_accounts
set granted_scopes = case
      when coalesce(array_length(granted_scopes, 1), 0) > 0 then granted_scopes
      when coalesce(raw->>'scope','') <> '' then regexp_split_to_array(btrim(raw->>'scope'), E'\\s+')
      else '{}'::text[]
    end,
    oauth_reconnect_required = not (
      'https://www.googleapis.com/auth/gmail.send' = any(case when coalesce(array_length(granted_scopes, 1),0)>0 then granted_scopes else regexp_split_to_array(btrim(coalesce(raw->>'scope','')), E'\\s+') end)
      and 'https://www.googleapis.com/auth/gmail.readonly' = any(case when coalesce(array_length(granted_scopes, 1),0)>0 then granted_scopes else regexp_split_to_array(btrim(coalesce(raw->>'scope','')), E'\\s+') end)
      and 'https://www.googleapis.com/auth/gmail.settings.basic' = any(case when coalesce(array_length(granted_scopes, 1),0)>0 then granted_scopes else regexp_split_to_array(btrim(coalesce(raw->>'scope','')), E'\\s+') end)
    );


-- Expand lead suppression states used by inbound Gmail classification.
alter table if exists public.businesses drop constraint if exists businesses_status_check;
alter table if exists public.businesses add constraint businesses_status_check check (status in ('pending','scanning','found','ready','review','contacted','responded','no_inbox','bounced','invalid','duplicate','archived','blocked','unsubscribed','do_not_contact','sent'));

insert into public.scout_schema_versions(version, notes)
values ('10.40.0', 'Full Gmail sending, Scout-thread reply collection, delivery classification, Gmail-native signature synchronization, staged follow-ups, and schema guard.')
on conflict (version) do update set applied_at = now(), notes = excluded.notes;

select
  'READY'::text as scout_database_status,
  '10.40.0'::text as deployed_schema_contract,
  exists(select 1 from public.scout_schema_versions where version='10.40.0') as schema_version_recorded,
  to_regclass('public.gmail_accounts') is not null as gmail_accounts_ready,
  to_regclass('public.reply_history') is not null as reply_history_ready,
  to_regclass('public.no_inbox_records') is not null as delivery_records_ready,
  to_regprocedure('public.get_due_followups(uuid,integer,text,integer,integer,uuid,text)') is not null
    or exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='get_due_followups') as due_followups_ready,
  exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='count_due_followups') as count_due_followups_ready,
  exists(select 1 from information_schema.columns where table_schema='public' and table_name='gmail_accounts' and column_name='granted_scopes') as oauth_scope_columns_ready,
  exists(select 1 from information_schema.columns where table_schema='public' and table_name='gmail_accounts' and column_name='last_reply_sync_at') as reply_sync_columns_ready,
  exists(select 1 from information_schema.columns where table_schema='public' and table_name='gmail_accounts' and column_name='gmail_signature_synced_at') as signature_sync_columns_ready;
...
-- BEGIN SCOUT V10.42.2 PENDING DELETE / CANCELLED IMPORT UNLOCK
...
insert into public.scout_schema_versions(version, applied_at, notes)
values ('10.42.2', now(), 'Pending no-email delete RPC and cancelled-import UI unlock')
on conflict (version) do update set applied_at=excluded.applied_at, notes=excluded.notes;
notify pgrst, 'reload schema';
-- END SCOUT V10.42.2 PENDING DELETE / CANCELLED IMPORT UNLOCK
...
-- BEGIN SCOUT V10.42.6 SENDING STABILITY FIX
-- =============================================================================
...
-- END SCOUT V10.42.6 SENDING STABILITY FIX
